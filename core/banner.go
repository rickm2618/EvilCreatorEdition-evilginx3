...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by Kuba Gretzky (") + handleClr.Sprintf("@mrgretzky") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION + textClr.Sprintf("Evil Creator Edition"))
	fmt.Fprintf(color.Output, "%s", txt)
}
