					...
					if ok && !s.IsDone {
						for _, au := range pl.authUrls {
							if au.MatchString(req.URL.Path) {
								err := p.db.SetSessionTokens(ps.SessionId, s.CookieTokens)
								if err != nil {
									log.Error("database: %v", err)
								}
								if err == nil {
					...
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { 
								...
								is_auth = s.addCookieAuthToken(c_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, auth_tokens)
								if len(pl.authUrls) > 0 {
									is_auth = false
								}
								if is_auth {
									if err := p.db.SetSessionTokens(ps.SessionId, s.CookieTokens); err != nil {
										log.Error("database: %v", err)
									}
									s.IsDone = true
								}
